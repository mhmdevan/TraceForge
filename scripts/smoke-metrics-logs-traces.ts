...
async function main(): Promise<void> {
  ...
  console.log("metrics+logs+traces smoke check passed");
}

async function checkTraceLogCorrelation(): Promise<void> {
  const correlationId = `phase6-${randomUUID()}`;
  ...
  const createResponse = await fetch(`${services[0].url}/transactions`, {
    ...
    body: JSON.stringify({
      userId: "metrics-logs-traces-smoke-user",
      amount: 12.5,
      currency: "USD",
  ...
  if (createResponse.status !== 201) {
    throw new Error(`trace smoke create returned ${createResponse.status}`);
  }
  ...
  await retry(async () => {
    ...
    if (!traceId) {
      throw new Error(`loki logs for ${correlationId} do not include trace_id yet`);
    }
